...
class COWCFRCNNTrainer:
    ...
    def data_loaders(self):
        ...
        dataset = COWCFRCNNDataset(root=self.config['path']['data_dir_HR'],
    ...
    def train(self):
        ...
        data_loader, dataset_test, _, _, _, _, _, _, _ = self.data_loaders()
        ...
        for epoch in range(num_epochs):
            ...
            evaluate_base(model, dataset_test, device=self.device)
            if epoch % 10 == 0:
                self.save_model(model, 'FRCNN_model_multi_class_cowc_HR', epoch)
